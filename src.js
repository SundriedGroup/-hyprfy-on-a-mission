...
async function intelligence(){
  await renderIntelligence(sb,view,{platform:"instagram",days:30});
}
...
function render(){
  if(!S.session){
    view.innerHTML=`<section class=page><h2>SIGN IN</h2><form id=login><input id=email type=email placeholder=Email required><input id=pw type=password placeholder=Password required><button>Sign in</button></form></section>`;
  ...
  if(S.tab==="stats")return renderSocialStats(view,sb);
