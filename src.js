...
function shell(){A().innerHTML=`<div class=appframe><aside class=sidebar><div class=brandmark><span>HYPRFY</span><strong>ON A MISSION</strong><small>PRODUCTION OS</small></div><nav class=sidenav><small>WORKSPACE</small><button data-tab=weeks data-weekview=day class=${S.tab==="weeks"&&S.view==="day"?"active":""}>Today</button><button data-tab=weeks data-weekview=board class=${S.tab==="weeks"&&S.view==="board"?"active":""}>Week</button><button data-tab=weeks data-weekview=posted class=${S.tab==="weeks"&&S.view==="posted"?"active":""}>Pipeline</button><button data-tab=calendar class=${S.tab==="calendar"?"active":""}>Calendar</button><small>TOOLS</small><button data-tab=intelligence class=${S.tab==="intelligence"?"active":""}>Intelligence</button><button data-tab=moments class=${S.tab==="moments"?"active":""}>Moments</button></nav><div class=sideweek>${S.weeks.length?`<small>CURRENT WEEK</small><strong>W${String(S.wi+1).padStart(2,"0")} · ${ds(S.weeks[S.wi]?.start_date||S.weeks[0].start_date)}</strong>`:""}</div></aside><div class=mobilebar><div><strong>HYPRFY</strong><small>ON A MISSION</small></div><button id=menutoggle>☰</button></div><main class=mainarea><div id=view></div></main></div>`;document.querySelectorAll("[data-tab]").forEach(b=>b.onclick=()=>{S.tab=b.dataset.tab;if(b.dataset.weekview)S.view=b.dataset.weekview;shell();render()});if($("menutoggle"))$("menutoggle").onclick=()=>document.querySelector(".sidebar")?.classList.toggle("open")}
...
function isToday(start,i){return iso(start,i)===new Date().toLocaleDateString("en-CA")}
...
function top(w){return`<div class=weekbar><div class=weekpicker>${S.weeks.map((x,i)=>`<button data-w=${i} class=${i===S.wi?"active":""}>W${String(i+1).padStart(2,"0")} <small>${ds(x.start_date)}</small></button>`).join("")}<button id=new>+ NEW</button></div><div class=weekcontext><span>${E(w.theme)}</span><small>${ds(w.start_date)} — ${ds(w.start_date,6)}</small></div></div>`}
...
function modernShootSheet(w,d,pr){const ig=feedPlan(d)||{},captured=pr.captured||[],shots=Array.isArray(ig.shots)?ig.shots:[],lines=scriptLines(ig),total=Math.max(shots.length,(d.film||[]).length),pct=total?Math.round(captured.length/total*100):0,events=S.events.filter(e=>e.date===iso(w.start_date,S.di));return`<div class=productionlayout><div class=productionmain><section class=posthero><div><span>TODAY'S POST</span><h2>${E(ig.concept||d.tag||d.title)}</h2><div class=formatline>${E(ig.fmt||"STORIES / CAPTURE")}</div>${ig.viewer_value?`<p>${E(ig.viewer_value)}</p>`:""}</div><div class=heroaction><small>NEXT SHOT</small><strong>${E(shots.find((_,i)=>!captured.includes(i))||shots[0]||(d.film||[])[0]?.line||"Capture what happens")}</strong></div></section><section class=modernsection><header><div><span>01</span><h3>Shot list</h3><small>What you need to get</small></div><b>${captured.length}/${total}</b></header><div class=modernshots>${(shots.length?shots:(d.film||[]).map(x=>x.line)).map((x,i)=>`<div class="modernshot ${captured.includes(i)?"done":""}"><button data-cap=${i}>${captured.includes(i)?"✓":"○"}</button><span>${E(x)}</span></div>`).join("")||'<p class=dim>No shots required.</p>'}</div></section><section class=modernsection><header><div><span>02</span><h3>Script + visuals</h3><small>What to say · what to show</small></div></header>${lines.length?`<div class=modernscript>${lines.map((line,i)=>`<article><div class=linecopy><small>${i===0?"TO CAMERA":"LINE "+String(i+1).padStart(2,"0")}</small><p>“${E(line)}”</p></div><div class=linevisual><small>SHOW</small><p>${E(visualForLine(line,i,ig))}</p></div></article>`).join("")}</div>`:`<div class=emptyinside>No fixed script. Capture the story naturally.</div>`}</section><section class=modernsection><header><div><span>03</span><h3>Post</h3><small>Cover + caption</small></div></header><div class=postcontent>${ig.cover_text?`<div class=coverblock><small>COVER</small><strong>${E(ig.cover_text)}</strong></div>`:""}<div class=captionblock><small>CAPTION</small>${ig.cap?`<p>${NL(ig.cap)}</p><button class=copybtn data-copy=caption>COPY</button>`:'<p class=dim>Write after capture.</p>'}</div></div></section></div><aside class=productionrail><section class=railcard><span>PROGRESS</span><div class=progressbar><i style="width:${pct}%"></i></div><strong>${captured.length} / ${total} shots</strong></section>${events.length?`<section class=railcard><span>TODAY</span>${events.map(e=>`<div class=railevent><strong>${E(e.title)}</strong>${e.description?`<p>${E(e.description)}</p>`:""}</div>`).join("")}</section>`:""}<section class=railcard><span>READY</span><div class=railchecks><div class=${total&&captured.length>=total?"done":""}>${total&&captured.length>=total?"✓":"○"} Shots</div><div class=${lines.length?"done":""}>${lines.length?"✓":"○"} Script</div><div class=${ig.cap?"done":""}>${ig.cap?"✓":"○"} Caption</div></div></section>${Array.isArray(ig.stories)&&ig.stories.length?`<details class=railstories><summary>STORIES <b>${ig.stories.length}</b></summary>${ig.stories.map(x=>`<div>○ ${E(x)}</div>`).join("")}</details>`:""}</aside></div>`}
function day(w){let d=w.days[S.di],p=S.ch==="youtube"?d.youtube:d.post?.[S.ch],pr=prod(d);let editor="";if(S.editing&&p){let base=S.ch==="youtube"?"youtube":`post.${S.ch}`;editor=`<div class=editor><h3>EDIT CONTENT</h3>${editField("FORMAT",base+".fmt",p.fmt,false)}${editField("CONCEPT",base+".concept",p.concept,false)}${editField("VIEWER VALUE",base+".viewer_value",p.viewer_value)}${editField("IDEA / PLAN",base+".plan",p.plan)}${editField("TO CAMERA / VOICEOVER",base+".spoken_line",p.spoken_line||p.script)}${editField("COVER TEXT",base+".cover_text",p.cover_text)}${editField("ON-SCREEN TEXT",base+".onscreen",p.onscreen)}${editField("POST COPY",base+".cap",p.cap)}<div class=editactions><button id=saveedit>SAVE CHANGES</button><button id=canceledit>CANCEL</button></div></div>`}let prev=(S.di+6)%7,next=(S.di+1)%7;return`${top(w)}<section class=modernhead><div><span class=eyebrow>${DN[S.di]} · ${ds(w.start_date,S.di)}</span><h1>${E(d.title)}</h1><p>${E(d.tag||"")}</p></div><div class=daynav><button data-d=${prev}>← ${DN[prev]}</button><button data-d=${next}>${DN[next]} →</button></div></section><div class=metarow><div class=statuscompact>${["planned","captured","edited","published"].map(x=>`<button data-status=${x} class=${pr.status===x?"active":""}>${x}</button>`).join("")}</div><div class=channelcompact>${CH.filter(x=>x!=="youtube"||S.di===6).map(x=>`<button data-c=${x} class=${x===S.ch?"active":""}>${x}</button>`).join("")}</div><button id=editcontent class=quietbtn>${S.editing?"EDITING":"EDIT PLAN"}</button></div><div class=adaptivecompact>${checkinBox(w,d)}</div>${S.editing?editor:(S.ch==="instagram"?modernShootSheet(w,d,pr):`<section class=worksection>${post(p,S.ch)}</section>`)}`}
function posted(w){
